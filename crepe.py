...
# ==========================================
    ...
    if not st.session_state.cart:
        ...
        for x in st.session_state.cart:
            ...
            # จัดฟอร์แมตข้อความให้อ่านง่ายใน Discord
    ...
    # ... (โค้ดเดิมของคุณ: ส่วนที่ดึงค่าจากช่องกรอกข้อมูล) ...
    # ... (โค้ดเดิมของคุณ: ส่วนที่สร้างตัวแปร data) ...
    ...
    # -------------------------------------------------------
    # 👇👇 เอาโค้ด "อันแรก" มาแทรกตรงนี้ (ต่อท้ายเลย) 👇👇
    # -------------------------------------------------------
    
            st.cache_data.clear()   # 1. สั่งล้างความจำเก่าทิ้ง
            st.success("บันทึกสำเร็จ!") 
            import time
    
            time.sleep(1)           # 2. รอ 1 วินาทีให้ข้อความขึ้น
            st.rerun()              # 3. สั่งโหลดหน้าใหม่
            
            if payment_method == "โอน/สแกน" and uploaded_slip is None:
                st.error("❌ กรุณาแนบสลิปก่อนครับ")
            else:
                with st.spinner('กำลังส่งออเดอร์...'):
                    # 1. บันทึก Google Sheet (ถ้ามีฟังก์ชันนี้แล้ว)
                    try:
                        save_order_to_google_sheet(st.session_state.cart, total, payment_method)
                    except:
                        pass 

                    # 2. ส่งเข้า Discord Webhook (ใส่ลิงก์ให้แล้วครับ ✅)

        ...
        if st.button("ยืนยันการสั่งซื้อ", type="primary", use_container_width=True):
            ...
            if payment_method == "โอน/สแกน" and uploaded_slip is None:
                ...
                with st.spinner('กำลังส่งออเดอร์...'):
                    ...
                    try:
                        # [A] สร้างข้อมูล order_data สำหรับ Google Sheets
                        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        items_str = ", ".join([f"{item['name']}" for item in st.session_state.cart])
                        
                        # สร้าง DataFrame (นี่คือตัวแปรที่หายไป!)
                        ...
                        # [B] บันทึกลง Google Sheets
                        conn = st.connection("gsheets", type=GSheetsConnection)
                        conn.create(worksheet="Order", data=order_data)
                        
                        # [C] ส่งเข้า Discord (ต้องทำก่อน Rerun)
                        ...
                        st.cache_data.clear()      # ล้าง Cache ตามสูตร
                        
                        st.balloons()
                        st.success("✅ สั่งเรียบร้อย! ข้อมูลส่งเข้าครัวแล้ว")
                        
                        # [E] หน่วงเวลาและรีเฟรชหน้า (ต้องทำเป็นสิ่งสุดท้าย!)
                        time.sleep(2)
                        navigate_to('home')
